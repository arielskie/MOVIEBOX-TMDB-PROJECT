document.addEventListener("DOMContentLoaded", () => {
    // ===================================================================
    // 0. GLOBAL EVENT LISTENER FOR SCROLL BUTTON CLICKS
    // ===================================================================
   document.addEventListener("click", function (e) {
      // Check if a scroll button was clicked
      if (e.target.classList.contains("scroll-btn") || e.target.classList.contains("cast-scroll-btn") || e.target.classList.contains("tmdb-scroll")) {
          ...
          if (scrollContainer) {
              ...
              // --- THIS IS THE FIX ---
              // Scroll by the exact visible width of the container for a "page snap" effect
    ...
    // ===================================================================
    // HELPER FUNCTION FOR SCROLL BUTTONS
    // ===================================================================
    function manageScrollButtons(wrapperElement) {
        const scrollContainer = wrapperElement.querySelector('.cast-scroll, .tmdb-row');
        const leftBtn = wrapperElement.querySelector('.scroll-btn.left, .cast-scroll-btn.left, .tmdb-scroll.left');
        const rightBtn = wrapperElement.querySelector('.scroll-btn.right, .cast-scroll-btn.right, .tmdb-scroll.right');

        if (!scrollContainer || !leftBtn || !rightBtn) return;

        const observer = new ResizeObserver(() => {
            const isOverflowing = scrollContainer.scrollWidth > scrollContainer.clientWidth + 5; 
            if (!isOverflowing) {
    ...
    function initHomepage(API_KEY) {
        ...
        function getMediaType(item, endpointPath) {
            if (item.media_type && (item.media_type === 'movie' || item.media_type === 'tv')) { return item.media_type; }
            if (endpointPath.startsWith("tv/") || endpointPath.startsWith("discover/tv") || endpointPath.startsWith("trending/tv")) { return "tv"; }
            return "movie";
        }
       async function initSlider(rawEndpoint, apiKey) {
           const slider = document.getElementById("tmdb-slider");
            ...
            try {
                ...
                if (items.length === 0) { slider.innerHTML = "<p>No slider content found.</p>"; return; }
                ...
                for (let i = 0; i < items.length; i++) {
                    ...
                    const captionContent = bestLogoPath ? `<img class="logo-title" src="https://image.tmdb.org/t/p/w500${bestLogoPath}" alt="${title} Logo" />` : `<h2>${title}</h2>`;
                ...
                if (slider) slider.innerHTML = "<p style='color:red;'>Error loading slider.</p>";
            }
       }
            ...
            document.querySelectorAll(".widget-content").forEach((contentEl) => {
                ...
                const observer = new IntersectionObserver(async (entries, obs) => {
                    for (const entry of entries) {
                        ...
                        // --- THE FIX IS HERE ---
                        try {
                            // 1. Create a promise for a minimum delay
                            const delayPromise = new Promise(resolve => setTimeout(resolve, 500));

                            // 2. Create a promise for fetching the data
                            const fetchUrl = buildApiUrl(endpointPath, apiKey);
                            const fetchDataPromise = fetch(fetchUrl).then(res => res.json());

                            // 3. Wait for BOTH the delay and the fetch to complete
                            const [_, data] = await Promise.all([delayPromise, fetchDataPromise]);
        ...
        if (sliderEndpoint) { initSlider(sliderEndpoint, apiKey); }
    ...
    function initDetailsPage(API_KEY) {
        ...
        async function renderSeasons(seasons) {
            ...
            validSeasons.forEach((season) => {
                ...
                btn.dataset.airDate = season.air_date || ''; 
                btn.dataset.posterPath = season.poster_path || '';
                
                btn.textContent = `S${season.season_number}`;
                btn.onclick = () => { 
                    document.querySelectorAll(".season-btn").forEach(b => b.classList.remove("active")); 
                    btn.classList.add("active"); 
                    renderEpisodes(btn.dataset.seasonNumber, btn.dataset.airDate, btn.dataset.posterPath); 
                };
                seasonBtnWrap.appendChild(btn);
            });

            const firstSeasonButton = seasonBtnWrap.querySelector('.season-btn');
            if (firstSeasonButton) {
                firstSeasonButton.classList.add("active");
                renderEpisodes(firstSeasonButton.dataset.seasonNumber, firstSeasonButton.dataset.airDate, firstSeasonButton.dataset.posterPath);
            }
        }

        async function renderEpisodes(seasonNum, airDate, seasonPosterPath) {
            ...
            const trailerBtn = document.querySelector('.poster-play-btn');
            if(trailerBtn) {
                trailerBtn.dataset.season = seasonNum;
            }

            if (titleEl) {
                titleEl.textContent = `${titleEl.dataset.originalTitle} S${seasonNum}`;
            }
            if (metaEl && airDate) {
                const seasonYear = airDate.split('-')[0] || "N/A";
                const metaParts = metaEl.innerHTML.split('<span>|</span>');
                if (metaParts.length > 1) {
                    metaParts[1] = ` ${seasonYear} `;
                    metaEl.innerHTML = metaParts.join('<span>|</span>');
                }
            }
            if (posterImg) {
                if (seasonPosterPath && seasonPosterPath !== 'null') {
                    posterImg.src = `${IMG_URL}${seasonPosterPath}`;
                } else {
                    posterImg.src = posterImg.dataset.mainPoster; 
                }
            }
        ...
        window.goToPlayer = function() {
            ...
            if (id && type) {
                const playerUrl = (type === 'tv')
                    ? `/p/player.html?id=${id}&type=tv&season=${currentSeason}&ep=1`
                    : `/p/player.html?id=${id}&type=movie`;
        ...
        document.addEventListener("click", async function(e) {
            if (e.target.closest(".poster-play-btn")) {
                ...
                const movieType = btn.getAttribute("data-type");
                ...
                try {
                    let videoUrl;
                    if (movieType === 'tv' && seasonNum) {
                        videoUrl = `${BASE_URL}/tv/${movieId}/season/${seasonNum}/videos?api_key=${API_KEY}&language=en-US`;
                    } else {
                        videoUrl = `${BASE_URL}/${movieType}/${movieId}/videos?api_key=${API_KEY}&language=en-US`;
                    }
                    ...
                    if (trailer) { 
                        ...
                        trailerContainer.innerHTML = "<p>No trailer found for this season.</p>"; 
                    }
   ...
   // ===================================================================
        ...
        function populateFooter(info) {
            ...
            fetch(`https://api.themoviedb.org/3/${number_of_episodes ? "tv" : "movie"}/${info.id}/credits?api_key=${API_KEY}`).then(res => res.json()).then(data => {
                ...
                if (cast?.length > 0) {
                    ...
                    manageScrollButtons(castWrapper.querySelector('.cast-scroll-wrapper'));
                }
    ...
    function initializeApp(apiKey) {
        ...
        } else if (document.getElementById("tmdb-slider")) {
            initHomepage(apiKey);
        }
    }
